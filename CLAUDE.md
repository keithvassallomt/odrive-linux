...
**`nautilus_extension.py`** plugs into Nautilus's `MenuProvider` (right-click) and `InfoProvider` (per-file decoration). On right-click it inspects selected files: `.cloud`/`.cloudf` get a "Sync with odrive" item, regular files inside a known mount get an "Unsync" item. `update_file_info` paints emblems based on three states (matches macOS/Windows odrive): `odrive-syncing` if the path (with any `.cloud(f)` suffix stripped) is in the `sync_in_progress` table, otherwise `odrive-synced` on every materialized item inside any mount (skipping the mount root itself), otherwise no emblem on placeholders since their cloud-file-type icon already conveys "remote". The syncing check reads SQLite directly with a 0.5s TTL cache; the GUI also touches the folder's mtime on mark/clear so Nautilus's directory-listing inotify fires and the emblem flip is near-instant rather than waiting on the cache TTL. The extension also opportunistically pads zero-byte placeholders with a single null byte (see "Double-click intercept" below). Both shell out to `odrive-cli`, located via `_find_cli`: `$ODRIVE_CLI` override → `$PATH` lookup → `target/release/odrive-cli` → `target/debug/odrive-cli` (relative to the extension file). If none resolve, the extension loads but stays inert and prints a one-shot stderr hint at init. The mount list is discovered at extension init via `odrive-cli mounts --paths` and cached for the lifetime of the Nautilus process — restart Nautilus (`nautilus -q`) to pick up newly-added mounts or extension-code edits. The `odrive-locked` emblem is installed but not yet applied — wired up when we have a way to detect upstream-locked files.
